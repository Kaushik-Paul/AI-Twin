...
- [x] **Automated Resume Delivery**: if a user expresses interest in my resume, the agent asks for their email address and automatically sends it as a PDF attachment via Mailjet.
- [x] **Unanswered-question logging**: whenever the agent cannot confidently answer a question from the provided context, it uses a tool to log that question so it can be reviewed and addressed later.
